...
sub is_valid_division_promise_file_path {
    my $self = shift;
    my $path = shift // die "No path defined";
    my $file = shift // die "No file defined";
...
sub assert_no_division_name_collision {
    my $self     = shift;
    my $divname  = shift // die "No division name defined";
    my $relpath  = shift // die "No relative path defined";
    
    my $otherpath = $self->{divisions}->{$divname};
    if (defined $otherpath) {
	die "Name collision between for division '$divname' in paths '$relpath' and '$otherpath'";
    }
}
...
sub find_division_promise_files {
    ...
    die "Errors while finding division promise files:\n".join("\n",errors())
	if $self->errors();
}

sub parse_cfdivisions_bundlesequence_token {
    my $self     = shift;
    my $line     = shift // die "No line defined";
    my $division = shift // die "No division name defined";
    ...
    if ($@) {
	die "Parsing '*cfdivisions_bundlesequence' failed. $@";
    }
...
sub parse_cfdivisions_depends_token {
    ...
    if ($@) {
	die "Parsing '*cfdivisions_depends' failed. $@";
    }
...
sub read_division_promise_file {
    ...
    open(my $fh,"<",$path) || die "Could not open division promise file: $path";
    while (<$fh>) {
